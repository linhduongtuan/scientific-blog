    ...
    io.on('connection', (socket) => {
      ...
      socket.on('send_message', async (data) => {
        try {
          ...
          console.log('📤 Formatted message to send:', messageToSend)
          
          // Explicitly send to all connected sockets in the room, including the sender
          // This ensures everyone gets the message through socket events
          io.to(roomId).emit('receive_message', messageToSend)
          console.log('📡 Message broadcasted to room:', roomId, 'with content:', message.content.substring(0, 30))
